...
- スタッフ本人の登録・編集は、独立した組織単位のスタッフ管理画面で行う。
- スタッフ管理画面の一覧対象は`staff`ロール保有者とし、
  `staff`兼`shift_manager`は含めるが、`shift_manager`専業と
  `system_admin`専業は含めない。
- シフト管理者の一覧・ロール・担当店舗は、システム管理者専用の
  シフト管理者管理画面で扱う。
- システム管理者は、`staff`ロールを持たない専任のシフト管理者の
  `users`レコードを同画面から新規登録・編集できる。
- 店舗管理画面は`users`を作成・更新せず、`store_user`による所属だけを更新する。
- `shift_manager`によるスタッフ管理の範囲は同一組織全体とし、
  `store_shift_manager`の担当店舗では絞り込まない。
- 店舗情報、シフト編集、配布の認可だけを`store_shift_manager`で制限する。
...
- `email`をシステム全体で一意にする
- アプリケーションは保存前に`email`の前後空白を除去し、小文字へ正規化する
...
- スタッフ本人の新規登録、氏名・メールアドレス等の編集、在籍状態、
  `staff`ロール、勤務可能店舗、パスワード設定・再設定は、独立した
  組織単位のスタッフ管理画面で扱う
- `shift_manager`ロールと担当店舗割り当ては、システム管理者専用の
  シフト管理者管理画面で扱う
- 店舗管理画面から`users`レコードを新規作成しない
- 店舗管理画面から氏名、メールアドレス、パスワード、在籍状態、ロールを変更しない
- 店舗管理画面では、同一組織に存在する`staff`と店舗の
  `store_user`関係だけを追加・解除する
- 在籍状態の変更、ロール解除、所属解除で既存`shifts`・
...
- `trim`と小文字化を行った正規化済みメールアドレスで確認する
- Soft Delete済みユーザーも重複確認対象に含める
- 同一組織に既存ユーザーがいる場合は新規作成せず、
  既存ユーザーの編集または勤務可能店舗追加へ誘導する
- 別組織に同じメールアドレスが存在する場合も、
  現行のシステム全体一意制約により新規作成しない
- 別組織ユーザーの氏名、組織、在籍状態等はエラーへ含めない
- 同時登録ではDB一意制約違反も捕捉し、入力エラーとして返す
...
- 担当外店舗のシフトは閲覧専用とする
- スタッフ管理画面では、同一組織の`staff`ロール保有者を一覧・検索・閲覧できる
- 担当店舗や登録者にかかわらず、同一組織のスタッフを新規登録・編集できる
- 同一組織の任意の店舗について勤務可能店舗を追加・解除できる
- 新規登録時および編集時に`staff`ロールを付与できる
- `shift_manager`・`system_admin`ロールは付与・解除できない
- `system_admin`ロールを持つユーザーは編集できない
- 所属組織、店舗追加、シフト管理者の管理対象店舗は変更できない
...
- `staff`・`shift_manager`ロールおよびシフト管理者の担当店舗割り当てを変更できる
- シフト管理者管理画面では、`shift_manager`専業と`staff`兼任の両方を管理できる
- 同一組織の全スタッフの勤務可能店舗を変更できる
- `system_admin`ロールは通常のスタッフ管理画面から付与・解除しない
- 自分自身を非在籍にする変更、および同一組織の有効な
  `system_admin`が0人になる変更を拒否する
...
`role_user`の`staff`ロール変更はスタッフ管理画面で行い、
`shift_manager`ロール変更はシフト管理者管理画面で行う。
店舗管理画面の所属追加・解除から、ロールを自動付与・解除しない。

権限更新規則：

- `shift_manager`は`staff`ロールの付与だけを行える
- `shift_manager`は既存の`shift_manager`・`system_admin`ロールを
  同期処理で意図せず解除してはならない
- `system_admin`は`staff`・`shift_manager`を付与・解除できる
- `system_admin`ロールは通常の入力項目へ含めず、既存値を維持する
- Policyに加えて、対象ユーザーをロックしたService内でも再検証する
...
- 同じ`user_id`を異なる`store_id`へ登録できる
- `display_order`の初期値は`0`
- `is_active`の初期値は`true`
...
店舗管理画面の追加候補は、同一組織に登録済みで、有効な`staff`ロールを持ち、
対象店舗へ現在所属していないユーザーに限る。

他店舗へ所属しているユーザーも、対象店舗へ未所属であれば追加できる。

過去に同じ店舗との無効な所属レコードがある場合は、新しい行を追加せず、
既存の`store_user`を再び有効化する。

店舗管理画面は`users`を作成・更新せず、所属追加・解除によって
氏名、メールアドレス、パスワード、在籍状態、ロールを変更しない。
...
所属解除は対象の`store_id + user_id`関係だけに適用し、
他店舗の`store_user`、`users`、`shifts`、`published_shifts`を変更しない。

所属解除したスタッフは、その店舗における新しい月の通常候補から除外する。
所属解除前から存在する下書きと公開版は表示・編集・再配布に必要な履歴として維持する。

スタッフ管理画面と店舗管理画面は、所属の認可、対象組織の検証、
有効化、再有効化、解除、表示順設定を共通の所属更新Serviceへ集約する。
どちらの画面から更新しても、`store_user`以外の所属データを作成しない。

共通Serviceは店舗軸・スタッフ軸の一括同期自体を共通化せず、
一つの`store_id + user_id`関係について次を行う。
...
- 店舗管理画面から`users`を作成・更新せず、既存staffの所属だけを変更できる
- 他店舗所属中のstaffを別の店舗へ追加できる
- `store_id + user_id`の一意制約とアプリケーション側検証で二重所属を防止できる
- 所属解除が他店舗の所属、`users`、`shifts`、`published_shifts`を変更しない
- 店舗ごとのスタッフ表示順を保持できる
- シフト管理者が複数店舗を担当できる
- シフト管理者が選択した担当店舗へ有効な所属があるスタッフを表示できる
- シフト管理者がそのスタッフの対象月における全店舗シフトを閲覧できる
- シフト管理者が担当外店舗のシフトを編集できない
- シフト管理者が`staff`ロールだけを付与できる
- シフト管理者が`shift_manager`・`system_admin`ロールと担当店舗割り当てを変更できない
- システム管理者だけが`shift_manager`ロールと担当店舗割り当てを変更できる
- シフト管理者が未担当店舗を含む同一組織の全店舗へ勤務可能所属を設定できる
- 勤務可能店舗変更が`store_shift_manager`を変更しない
- 担当外店舗の編集を拒否できる
