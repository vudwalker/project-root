...
- スタッフ本人の登録・編集は、今後追加する組織単位のスタッフ管理画面で行う。
...
- スタッフ本人の新規登録、氏名・メールアドレス等の編集、在籍状態、
  `staff`・`shift_manager`ロール、所属店舗、パスワード再設定は、
  今後追加する組織単位のスタッフ管理画面で扱う
...
`role_user`の変更は、今後追加するスタッフ管理画面の権限管理で行う。
...
将来のスタッフ管理画面と現在の店舗管理画面は、所属の認可、対象組織の検証、
